...
userApp.post("/getTodo",expressAsyncHandler(async (request, response) => {
  let userCollectionObj = request.app.get("userObj");
  let newUserObj=request.body;
  ...
  let userOfDb = await userCollectionObj.updateOne(
    ...
    { $set: { todo:newUserObj.todo } }
   
   
  );
  console.log(userOfDb);
  if (userOfDb == null) {
    response.send({ message:"try again" });
  } else {
    response.send({ message:"added uccesfully!!" });
  }
